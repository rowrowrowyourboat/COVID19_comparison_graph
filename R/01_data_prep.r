...
create_groups<-function(idata, column_groups){
  ...
  mod_data<- mod_data %>% group_by(org, report_date )%>% summarize(result= sum(result ) )
...
world_data_death %<-% {
  tmp_world<- process_CSSEGIS("time_series_covid19_deaths_global.csv", 
                              c("Country.Region" , "Province.State" ),
                              c("Lat", "Long"),
                              "deaths"
  )
...
us_data_death %<-% {
  tmp_us<-process_CSSEGIS("time_series_covid19_deaths_US.csv", 
                          c("Country_Region" , "Province_State", "Admin2" ),
                          c("Lat", "Long_", "FIPS", "Combined_Key", "UID", "iso2", "iso3", "code3"  ),
                          "deaths"
  )
